...
import {saveProductTopics, updateProductTopics} from "../../../../api/api";
...
class DefineTopicClass extends React.Component {
    ...
    saveSubmit = async () => {
        ...
        form.validateFields().then((values) => {　　// 如果全部字段通过校验，会走then方法，里面可以打印出表单所有字段（一个object）
            ...
            if(this.props.detail.id){
                this.updateDefineTopicClass(values)
            }else{
    ...
    updateDefineTopicClass = async (values) => {
        values.productId=this.props.productInfo.id
        values.id=this.props.detail.id
        let listTemp=[];
        listTemp.push(values);
        updateProductTopics(listTemp).then(res => {
            if(res.status==1){
    ...
    addDefineTopicClass = async (values) => {
        ...
        let listTemp=[];
        listTemp.push(values);
        saveProductTopics(listTemp).then(res => {
            if(res.status==1){
                messageGlobal('success',res.msg);
                this.props.closeSubmit()
            }else{
